...
(re-frame/reg-sub
 ...
 :<- [:mediaserver/port]
 :<- [:initials-avatar-font-file]
 (fn [[contacts identity ens-name port font-file]]
   (let [contact (get contacts identity)]
     (cond-> contact
       (nil? contact)
       (enrich-contact identity ens-name port font-file)))))
